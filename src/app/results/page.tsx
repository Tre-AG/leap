...
type ResultsSection = "loading" | "profile" | "opportunities" | "quickWins" | "summary" | "nextSteps";
...
function ResultsContent() {
  ...
  const { state, reset, undoComplete, setRecommendations: cacheRecommendations } = useGame();
  const { characterClass, profile, recommendations: cachedRecommendations } = state;

  const initialSection = (): ResultsSection => {
    if (!cachedRecommendations) return "loading";
    const param = searchParams.get("section");
    if (param && ["profile", "opportunities", "quickWins", "summary", "nextSteps"].includes(param)) {
      return param as ResultsSection;
    }
    return "profile";
  };
  ...
  const [section, setSection] = useState<ResultsSection>(initialSection);
  ...
  useEffect(() => {
    ...
    if (cachedRecommendations) return;

    if (!fetchAttempted.current) {
      fetchAttempted.current = true;
      runFetch();
    }
  }, [characterClass, router, runFetch, cachedRecommendations]);
  ...
  const goPrev = () => {
    ...
    if (prev) {
      setSection(prev);
      if (prev === "quickWins" && recommendations) {
        setQuickWinIndex(recommendations.quickWins.length - 1);
      }
    }
  };
        ...
        <div className="mb-8 min-h-[400px]">
          ...
          {section === "summary" && (
            <ResultsSummary
              ...
              onReviewAnswers={() => router.push("/results/review")}
            />
          )}
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            className={`rounded-xl px-6 py-3 text-sm font-semibold transition-all ${
              section === "profile"
                ? "invisible"
                : "bg-white text-foreground/70 shadow-sm hover:shadow-md"
            }`}
          >
            Back
          </button>
